...
enum RUMProtocolV2 {
    ...
    enum MessageType: UInt8 {
        ...
        /// pubkey | legacy IK message-1 body]`. Receivers without
        /// `pqHybridKEM` capability MUST drop the frame; they should never
        /// receive it because the sender only emits this type when peer
        /// capabilities advertise PQ support.
    ...
    struct Capabilities: OptionSet, Sendable {
        ...
        // security. Engines must NEVER assert this bit unless
        // `SecurityConstants.doubleRatchetEnabled == true` AND
        // `DoubleRatchet` has been wired into the send/receive paths.

        /// Peer supports Double Ratchet transport-mode encryption.
        /// Negotiation rule: prefer DR iff BOTH sides advertise.
        /// When only one side advertises, the conversation stays on
        /// plain Noise transport mode (no PCS, but still has FS).
    ...
    enum CodecError: Error, Equatable {
        ...
        case wrongIDLength
        /// `signaturePresent` flag set but signature missing / wrong
        /// length. Thrown instead of `precondition` so a malformed
        /// envelope from a malicious peer doesn't crash the receiver.
    ...
    static func encode(_ envelope: Envelope) throws -> Data {
        ...
        if envelope.flags.contains(.signaturePresent) {
            // 🔐 BUG FIX (2026-05-10): swap `precondition` for a thrown
            // error. The encoder is reachable from re-broadcast paths
            // whose envelope content originates from peer devices /
            // server — a malicious peer setting `signaturePresent`
            // without the right-length signature would have aborted
            // every receiver mid-relay (process crash). Now the call
            // site catches `CodecError` and drops the malformed
            // envelope.
            guard envelope.signature?.count == signatureSize else {
                throw CodecError.malformedSignature
            }
        }
