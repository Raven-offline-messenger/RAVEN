...
final class RavenEnvelopeBridgeServiceTests: XCTestCase {
    ...
    @MainActor
    func testLocalDestinationDeliversToEndpointNotForward() async {
        FeatureFlag.ravenEnvelopeV1.setEnabled(true)
        ...
        let (packed, mid) = samplePacked()
        let exp = expectation(description: "endpoint ingest")
        let obs = NotificationCenter.default.addObserver(
            forName: .ravenEnvelopeV1EndpointIngest,
            object: nil,
            queue: .main
        ) { note in
            let got = note.userInfo?["messageId"] as? Data
            XCTAssertEqual(got, mid)
            exp.fulfill()
        }
        ...
        XCTAssertEqual(d, .deliverToEndpoint)
        await fulfillment(of: [exp], timeout: 2)
    }

    func testAckRelayDecisionAndOpaquePeek() {
        FeatureFlag.ravenEnvelopeV1.setEnabled(true)
        let sk = Curve25519.Signing.PrivateKey()
        ...
        body.append(acked)
        body.append(1)
        body.append(Data(repeating: 1, count: 12))
        body.append(contentsOf: withUnsafeBytes(of: UInt64(99).bigEndian) { Data($0) })
        ...
        env.sign(with: sk)
        XCTAssertEqual(
            RavenEnvelopeEndpointIngest.opaqueAckedMessageId(from: env),
            acked
        )
        XCTAssertEqual(
