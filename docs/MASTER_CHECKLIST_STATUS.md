...
> Formal §-by-§ walk **DOCUMENT COMPLETE** (60/60 rows) — see walk doc.  
...
| 21 | Terminal First-Run Flow | IMPLEMENTED | ash identity create / first-run on macOS |
| 22 | Terminal Main Menu | IMPLEMENTED | petname-first banner verified macOS |
| 23 | Messages Menu | IMPLEMENTED | ash messages path + demos |
| 24 | Chat History | IMPLEMENTED | chat_history.json + ash history |
| 25 | Send New Message | IMPLEMENTED | stdin / IPC send; no argv plaintext |
...
| 31 | NAT Traversal | BLOCKED_HARDWARE | Docker NAT sim SKIP (`docker_daemon_down`); live CGNAT/DCUtR open |
...
| 36 | Bluetooth Transport | IN_PROGRESS | framing + iOS carrier tests; headless GATT **BLOCKED_HARDWARE** |
| 37 | Bluetooth Forwarding Policy | IN_PROGRESS | software policy; physical multi-hop BLE **BLOCKED_HARDWARE** |
...
| 41 | Out-of-Order | IN_PROGRESS | ATSAM skipped-key iOS; Rust AEAD; full matrix incomplete |
...
| 47 | Cross-Platform Interop | IN_PROGRESS | macOS runtime; Win/Linux cross-compile; Linux container runtime blocked (Docker daemon) |
| 48 | Mandatory Network Tests | IMPLEMENTED | demos green; §59 harness network steps |
| 49 | Mandatory Security Tests | IMPLEMENTED | refuse argv, UDS peer-UID, KATs, anti-spam |
| 50 | Reliability and Scale Tests | IMPLEMENTED | harness + fuzz_smoke scale hooks (not multi-day soak) |
| 51 | Terminal-Specific Security Tests | IMPLEMENTED | ash/raven-node argv refuse + doctor redaction |
...
| 56 | Documentation | IMPLEMENTED | SERVERLESS_MODEL, INSTALL_*, walk doc (§1–60) |
...
- `docs/MASTER_CHECKLIST_WALK_IN_PROGRESS.md` — formal walk evidence (60/60)  
...
- `scripts/nat_docker_sim.sh` + `node/proof_artifacts/nat_docker_*`  
...
- Live secret rotation if any leaked credentials remain

### BLOCKED_HARDWARE
- Physical 3-phone / multi-NAT CGNAT / DCUtR
- Headless CoreBluetooth desktop radio
- Live rust-libp2p Kad on public Internet
- Docker NAT / Linux-in-container runtime (**daemon down** this session: `docker_daemon_down`)

### Software debt (IN_PROGRESS)
