...
public struct NoiseReplayWindow {
    ...
    /// Default mirrors `SecurityConstants.Replay.bitmapWindowSize` (1024).
    /// Kept as a literal because `SecurityConstants` is internal and
    /// cannot appear in a `public` default-argument expression — same
    /// constraint the iOS-target build hit and worked around the same way.
